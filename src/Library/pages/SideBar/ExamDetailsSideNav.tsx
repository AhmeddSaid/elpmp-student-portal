...
import {
	ExamDetailsSideNavAr,
	ExamDetailsSideNavEn,
} from "../../../../messages/ExamDetailsSideNav/ExamDetailsSideNav";
import CustomLink from "@/Library/Common/CustomLink/CustomLink";
import styles from "@/Library/Common/Grids/Spaces.module.css";
import DomainIcon from "@/Library/Common/Icongraphy/DomainIcon/DomainIcon";
import History from "@/Library/Common/Icongraphy/History/History";
...
import WhitePaeWithRight from "@/Library/Common/Icongraphy/WhitePaeWithRight/WhitePaeWithRight";
import WhitePage from "@/Library/Common/Icongraphy/WhitePage/WhitePage";
import SideBarDropDown from "@/Library/Common/SideBarDropDown/SideBarDropDown";
import { SideBarExamLink } from "@/Library/Common/SideBarDropDown/SideBarDropDown.styles";
...
export interface ExamItem {
	...
	SubExam: {
		...
		order?: number;
		durationPerQuestion?: string | number;
	};
}

const ExamDetailsSideNav = ({
	locale,
	data,
	exam,
}: {
	...
	const groupedBySubExamType = data?.reduce(
		(acc, item) => {
	...
	const t = locale === "en" ? ExamDetailsSideNavEn : ExamDetailsSideNavAr;
	...
	return (
		<SideBar collapse={false} locale={locale}>
			<MainLogo href={"/"}>
				{!isOpen ? (
					<svg
						width="26"
			...
			/>
			<SideBarExamLink>
				{/*<SideBarItem href={`/exam/${exam}`} icon={<LineHome />} title={t.home} />*/}
				<SideBarItem href={`/exam/${exam}/history`} icon={<History />} title={t.examHistory} />
				{/*<SideBarItem href={`/exam/${exam}/mistakes`} icon={<Invoice />} title={t.killMistakes} />*/}
				{groupedBySubExamType?.DOMAIN && groupedBySubExamType.DOMAIN.length > 0 && (
					<SideBarDropDown
						icon={<DomainIcon />}
						exam={exam}
						title={t.domain}
						// @ts-ignore
						subTitles={groupedBySubExamType.DOMAIN}
					/>
				)}
				{groupedBySubExamType?.KNOWLEDGEAREA && groupedBySubExamType.KNOWLEDGEAREA.length > 0 && (
					<SideBarDropDown
						icon={<WhitePage />}
						title={t.chapters}
						exam={exam}
						// @ts-ignore
						subTitles={groupedBySubExamType.KNOWLEDGEAREA}
					/>
				)}
				{groupedBySubExamType?.FULLEXAMS && groupedBySubExamType.FULLEXAMS.length > 0 && (
					<SideBarDropDown
						icon={<WhitePaeWithRight />}
						title={t.allExam}
						exam={exam}
						// @ts-ignore
						subTitles={groupedBySubExamType.FULLEXAMS}
					/>
				)}
				{groupedBySubExamType?.OTHER && groupedBySubExamType.OTHER.length > 0 && (
					<SideBarDropDown
						icon={<TestIcon />}
						title={t.other}
						exam={exam}
						// @ts-ignore
						subTitles={groupedBySubExamType.OTHER}
					/>
				)}
			</SideBarExamLink>
		</SideBar>
...
export default ExamDetailsSideNav;
